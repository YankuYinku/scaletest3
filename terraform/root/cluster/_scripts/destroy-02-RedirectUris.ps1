...
# Remove redirect Uri for portal api
...
if ($portalApiApplication.web.redirectUris.contains($portalApiRedirectUri)) {
    $portalApiApplication.web.redirectUris = $portalApiApplication.web.redirectUris | Where-Object { $_ -ne $portalApiRedirectUri }
}
...
# Remove redirect Uris for portal api
...
if ($portalRootApplication.spa.redirectUris.contains($portalRootRedirectRootUri)) {
    $portalRootApplication.spa.redirectUris = $portalRootApplication.spa.redirectUris | Where-Object { $_ -ne $portalRootRedirectRootUri }
}
...
if ($portalRootApplication.spa.redirectUris.contains($portalRootRedirectSignupUri)) {
    $portalRootApplication.spa.redirectUris = $portalRootApplication.spa.redirectUris | Where-Object { $_ -ne $portalRootRedirectSignupUri }
}

...
# Set DNS Entry (public application gateway ip to dev-meinapetito.apebs.de)
...
$authorizationConnectionString = $(az keyvault secret show --vault-name "apetitoKeyVault-dev" --name "apetito-authorization-api-ConnectionStrings-AuthorizationDbConnectionString-meinapetito" --query "value")
...
$notificationConnectionString = $(az keyvault secret show --vault-name "apetitoKeyVault-dev" --name "apetito-notification-api-ConnectionStrings-NotificationContext-meinapetito" --query "value")
...
./configureApplicationGatewy.ps1
./addRedirectUri.ps1
#./addAzureServicePrincipalSecret.ps1
